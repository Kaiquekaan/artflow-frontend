...
import api from '../../api';
import SkeletonGridModal from './SkeletonGridModal';

const GridPostModal = ({ postId, initialImageIndex = 0, isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchPostData = async () => {
        ...
        try {
            ...
            console.log('dados do post 2:', response.data)
  ...
  if(isLoading) return <SkeletonGridModal/>
    ...
    if (replyingTo && replyContent) {
      ...
      try {
        ...
        const newComment = { 
          ...
          profile_picture_url: usertData.userdata.profile_picture_url,
          likes_count: 0,
...
const renderCaptionWithLinks = (caption) => {
  ...
  return (
      <div className={`modal-content ${post.media_files.length === 0 ? 'no-media' : ''} gridpage`} onClick={(e) => e.stopPropagation()}>
        {showLoadingBar && <div className="loading-bar"></div>}
        <div className='modal-user-grid-container'>
        <button className="modal-close-btn gridpage" onClick={() => handleback()}>
        <FontAwesomeIcon icon={faArrowLeft} />
        </button>
        <div className='grid-post-title'>Post</div>
        </div>
        <div className={`modal-left gridpage ${post.media_files.length === 0 ? 'no-media' : ''}`}>
        ...
        <div className={`modal-right gridpage ${post.media_files.length === 0 ? 'no-media' : ''}`}>
          ...
          {post?.caption &&(
          <div className="modal-caption gridpage">
